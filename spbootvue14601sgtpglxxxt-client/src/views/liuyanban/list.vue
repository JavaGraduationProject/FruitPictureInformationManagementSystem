        ...
        <el-card class="box-card">
            ...
            <el-table border :data="list" @sort-change="sortChange" style="width: 100%" highlight-current-row>
                ...
                <el-table-column prop="liuyanneirong" label="留言内容">
                    <template slot-scope="{row}"> {{ row.liuyanneirong }} </template>
                </el-table-column>
                <el-table-column prop="liuyanren" label="留言人" width="80">
                    <template slot-scope="{row}"> {{ row.liuyanren }} </template>
                </el-table-column>
                <el-table-column prop="huifuneirong" label="回复内容">
                    <template slot-scope="{row}"> {{ row.huifuneirong }} </template>
                </el-table-column>
                <el-table-column prop="liuyanzhuangtai" label="留言状态" width="80">
                    <template slot-scope="{row}"> {{ row.liuyanzhuangtai }} </template>
                </el-table-column>
                    ...
                    <template slot-scope="{row}">
                        <el-button-group>
                            <el-tooltip content="编辑" placement="top">
                                <el-button v-if="row.liuyanzhuangtai == '待回复'"  icon="el-icon-edit" @click="$goto({path:'/admin/liuyanbanupdt',query:{id:row.id } })" type="warning" size="mini"></el-button>
                            </el-tooltip>
    ...
    export default {
        ...
        methods: {
            ...
            loadList(page) {
                ...
                this.$post(api.liuyanban.list, filter)
                    .then((res) => {
                        this.loading = false;
            ...
            deleteItem(row) {
                this.$confirm("确定删除数据？", "提示", {
                    ...
                    .then(() => {
                        ...
                        this.$post(api.liuyanban.delete, {
                            // 提交后台
                            id: row.id,
                        })
                            .then((res) => {
